...
export default class DateRangeFilter extends React.Component {
  ...
  renderDateRanges() {
    return this.props.dateRanges.map((dateRange) => {
      return <option key={dateRange.get('id')} value={dateRange.get('id')}>{dateRange.get('name')}</option>;
    }).toJS();
  }
      ...
      <div className="row">
        <div className="col-xs-4">
          <div className="form-horizontal">
            <Input type="select" label="Date Range" defaultValue={this.props.currentDateRange.get('id')} labelClassName="col-xs-4" wrapperClassName="col-xs-8"
