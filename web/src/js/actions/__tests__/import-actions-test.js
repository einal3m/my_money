...
describe('ImportActions', () => {
  ...
  describe('getBankStatements', () => {
    ...
    it('calls the bank_statements api', () => {
      importActions.fetchBankStatements();

      expect(dispatcherSpy).toHaveBeenCalledWith({ type: importActions.GET_BANK_STATEMENTS });
    ...
    it('saves the import history to the store on success', () => {
      ...
      expect(dispatcherSpy).toHaveBeenCalledWith(
        { type: importActions.SET_BANK_STATEMENTS, bankStatements: ['transformedBankStatement'] }
      );
    });
  });
});
