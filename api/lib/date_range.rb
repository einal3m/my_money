module Lib
  ...
  class DateRange
    ...
    def self.valid?(klass)
      Lib.const_get(klass).new.is_a?(DateRange)
    rescue NameError
      false
    end
    ...
    def financial_year_from(years_ago = 0)
      Date.new(
        Date.today.month > 6 ? Date.today.year - years_ago : Date.today.year - 1 - years_ago, 7, 1
      )
    end

    def financial_year_to(years_ago = 0)
      Date.new(
        Date.today.month > 6 ? Date.today.year + 1 - years_ago : Date.today.year - years_ago, 6, 30
      )
    end
  end
