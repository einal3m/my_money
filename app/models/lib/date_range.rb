...
module Lib
  ...
  class DateRange
    ...
    def financial_year_from(years_ago = 0)
      Date.new(Time.zone.today.month > 6 ? Time.zone.today.year - years_ago : Time.zone.today.year - 1 - years_ago, 7,
               1)
    end

    def financial_year_to(years_ago = 0)
      Date.new(Time.zone.today.month > 6 ? Time.zone.today.year + 1 - years_ago : Time.zone.today.year - years_ago, 6,
               30)
    end
  end
  ...
  class CurrentMonthDateRange < DateRange
    def initialize(_args = {})
      ...
      @from_date = current_month_from_date
      @to_date = current_month_to_date
    end
