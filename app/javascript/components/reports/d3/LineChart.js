...
function createYAxis(vis, yScale, dim, callbacks) {
  ...
  yAxisG
    .selectAll("path.y-axis, .y-axis line, .y-axis path")
    .attr("fill", "none")
    .attr("stroke-width", "1px")
    .attr("stroke", "#ddd")
    .attr("shape-rendering", "crispEdges");

  yAxisG.selectAll("text").attr("x", -10).attr("text-anchor", "end");
}
...
function createXAxis(vis, xScale, dim, callbacks) {
  ...
  const xAxisG = vis
    ...
    .attr(
      "transform",
      `translate(${dim.leftMargin},${dim.topMargin + dim.chartHeight})`
    )
    .call(xAxis);

  xAxisG
    .selectAll("path.x-axis, .x-axis line, .x-axis path")
    .attr("fill", "none")
    .attr("stroke-width", "1px")
    .attr("stroke", "#ddd")
    .attr("shape-rendering", "crispEdges");

  xAxisG.selectAll("text").attr("y", 20);
}
...
function createHoverCircles(vis, seriesData, xScale, yScale, dim, callbacks) {
  ...
  const focusCircles = seriesData.map((series) => {
    ...
    group.append("circle").attr("r", 3.5).attr("fill", series.backgroundColour);
  ...
  function moveCircles() {
    const date = roundedDate(xScale.invert(d3.pointer(this)[0]));
    ...
    seriesData.forEach((series, seriesIndex) => {
      ...
      if (Number.isFinite(y)) {
        focusCircles[seriesIndex]
          .attr("transform", `translate(${xScale(date)},${yScale(y)})`)
          .style("display", null);
      } else {
        focusCircles[seriesIndex].style("display", "none");
      }
      values[seriesIndex] = Number.isFinite(y)
        ? callbacks.formatYLabels(y)
        : "--";
    });
...
function create0Axis(vis, yScale, dim) {
  if (yScale(0) >= 0 && yScale(0) <= dim.chartHeight) {
    const zeroAxis = vis
      ...
      .attr(
        "transform",
        `translate(${dim.leftMargin},${dim.topMargin + yScale(0)})`
      );
